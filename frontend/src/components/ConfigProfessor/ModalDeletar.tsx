...
export default function ModalMatricula(props: any) {
  const deletarProfessor = async (e: any) => {
    try {
      await fetch(`http://localhost:3333/professores/${props.professor}`, {
        method: "DELETE",
      });
      handleClose()
      location.reload()
        ...
        <Fade in={open}>
          <Box sx={style}>
            ...
            <p className="m-7 text-xl text-center">
              Este professor está prestes a ser excluído, deseja continuar e
              removê-lo?
            </p>
