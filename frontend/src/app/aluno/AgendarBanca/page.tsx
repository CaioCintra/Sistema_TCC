...
export default function AgendarBanca(props: any) {
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await axios.get(
          `http://localhost:3333/bancas/${aluno.ra}`
        );
      } catch (error) {
  ...
  const cadastrarBanca = async (e) => {
    ...
    try {
      ...
      if (response.status === 500) {
        ...
        try {
          await fetch(`http://localhost:3333/alunos/${aluno.ra}`, {
            method: "PUT",
  ...
  return (
    <div>
      ...
      <Box
        className="bg-[var(--background-color)] w-[70%] rounded-lg p-10 absolute shadow-lg shadow-black"
        sx={style}
      >
